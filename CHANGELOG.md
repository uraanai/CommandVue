...
- **Theme generation engine (Prompt 4 Phase B).**
  ...
  - [`docs/theme-generation-algorithm.md`](docs/theme-generation-algorithm.md) documents the per-scale math, contrast verification, gamut handling, limitations, where the engine refines the prompt's surface-elevation text to match the shipped built-ins, and an **Extending the vocabulary** decision tree for adding new component / semantic tokens when the app grows new controls. The `commandvue-theming-system` agent skill mirrors the 95% / 4% / 1% recipe under Common operations so future agents adding controls hit the right path without reading the full doc.
