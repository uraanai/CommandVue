...
- **Theme import / export UI + LLM schema docs + roadmap (Prompt 4 Phase G — closes Prompt 4).**
  - **`ThemeImportDialog.vue`** — import a `PortableTheme` JSON three ways: drag-and-drop a `.json` file, "Browse…" (native picker via `browser-fs-access`, the locked-stack file helper — no raw `<input type="file">`), or paste into a textarea. Live preview validation mirrors `importThemeFromJson`'s parse → schema-version → Zod steps so errors / the theme name show before committing; the actual Import calls the unchanged `importThemeFromJson` (single source of truth) which re-validates, applies the conflict policy, and auto-mints a ULID. Conflict policy is a segmented control (import a copy / replace existing / skip on clash; default "import a copy"). "Apply after importing" defaults on.
  - **Per-card actions in the Themes picker.** Each card now has Export (all themes — `downloadThemeFile`, built-ins included as starting points), Edit (generated themes only — opens the customizer pre-filled from the `generation` block), plus the existing Delete (custom only) and Apply. The picker mounts `ThemeImportDialog` + `ThemeCustomizerDialog` as siblings and gets an **Import…** button in its header.
  - **Menu integration.** `View → Import theme…` opens the import dialog directly (alternative to the picker route).
  - **`docs/theme-schema-for-llms.md`** — a paste-into-your-LLM prompt template: the `PortableTheme` envelope shape, the full set of overridable token keys (grouped: surfaces / text / borders / interactive / accent scale / status / typography), OKLCH value guidance, the four hard rules (allowlist-only keys, surface elevation, text contrast, semantic status hues), and a self-verification checklist. Registered in the VitePress sidebar under Look & feel.
  - **`docs/roadmap.md`** extended with the remaining deferred theming work: raw JSON editor with live preview, theme marketplace / community sharing, accessibility variants (high-contrast + CVD-safe), and typography/motion theming — each cross-linking the relevant design doc.
  - `browser-fs-access` added as a runtime dep (the locked-stack file-access helper) for the native file picker.
  - 4 doc-guard tests in `tests/unit/docs/theme-schema-for-llms.spec.ts` (balanced code fences; every token the doc mentions is in the known-token allowlist; required core tokens present; the example envelope is valid JSON with known token keys) so the LLM doc can't silently drift out of sync with `knownTokens.ts`.
  - **This closes Prompt 4** — the seven-phase runtime theme authoring system (storage → generation → registry → import/export core → customizer → workspace binding → import/export UI) is complete.
- **Phase F Stage-2 fixes — workspace dot colour + custom-theme delete.**
  - **Workspace-switcher dot no longer leaks the active theme onto other workspaces.** `dotColor` / `themeTooltip` previously fell back to `themeStore.currentThemeId` when a workspace's cached binding was the `null` "no-binding" sentinel — so every unbound workspace's dot rendered whichever workspace was currently active (e.g. switching to a blue workspace turned the purple workspace's dot blue too). `hydrateBindings` now always resolves each workspace's *effective* theme id (explicit binding → global pointer → fallback) and stores that; `dotColor` / `themeTooltip` read purely from the resolved map with a stable grey fallback, never `currentThemeId`. Each dot is now correct and stable regardless of which workspace is active.
  - **Custom themes can be deleted from the Themes picker.** Each non-built-in card now has a delete control using a two-click confirm (first click arms to a red "Delete?", second click deletes; `blur` disarms). Built-ins have no delete affordance (they're registered, not stored). Delete routes through `themeRepo.delete` — which clears workspace bindings (Phase A) and unregisters from the registry (Phase C); the picker's `subscribe` listener removes the card live. If the deleted theme is currently applied, the picker falls back to `compact-light` first so the app never sits on a deleted id.
- **Per-workspace binding for custom themes + paired-variant bridging (Prompt 4 Phase F).** The per-workspace binding contract from Prompt 3 already worked for any registered theme (built-in or custom — `themeStore.setWorkspaceTheme` takes a `ThemeId`, the registry doesn't distinguish source); the cleanup-on-delete chain was wired in Phase A (`themeRepo.delete` scans `appMetaRepo.getKeysByPrefix("commandvue:workspace-theme-")` and drops any binding pointing at the deleted theme). Phase F adds the two pieces still missing:
  - **`useTheme.bridgeVariant` is now `generation.paired`-aware.** Precedence: (1) check the current theme's `generation.paired` field — if the paired theme is registered and its `mode` matches the resolved mode, swap to it; (2) fall back to the Prompt 3 `-light` / `-dark` suffix swap for built-ins; (3) graceful no-op (data-theme attribute still flipped, dark-mode CSS rules still take effect). Means the Light/Dark/Auto toggle bridges seamlessly for generated themes that the customizer cross-linked via the "Generate paired variant" checkbox — regardless of their ULID ids.
  - **`WorkspaceSwitcher` theme indicator dot now works for all theme sources.** Used to fall back to grey for anything not in the built-in static map (which was the entire generated/imported population). Now resolves the color from the registered theme: built-ins use the static map (their tokens are `var()`-references that don't paint outside the active cascade), generated themes use `theme.generation.accentColor` directly, imported themes use `theme.tokens["--color-interactive"]` when it's a literal value.
  - **`themeStore` cache invalidation on theme deletion.** Phase A's `themeRepo.delete` already cleared the IDB binding (so `resolveForWorkspace` returns the correct fallback) and Phase C's registry sync unregistered the deleted theme, but `themeStore.workspaceBindings` is an in-memory write-through cache that wasn't getting notified — so the workspace-switcher dot could briefly render the deleted theme's accent until the next workspace switch. `themeStore` now subscribes to `themeRegistry` and prunes any cached binding pointing at an unregistered id; also clears `currentThemeId` if the active theme was deleted out from under it, so reactive consumers see the change immediately.
  - 4 new tests in `tests/unit/composables/useTheme.spec.ts` covering: `generation.paired` drives the swap for generated themes; suffix swap still works for built-ins (no regression); no-op when current theme already matches resolved mode; no-op when no theme is applied. Stage 1 Playwright run additionally verified the cache invalidation end-to-end (`getWorkspaceBinding` returns `null` after delete; `currentThemeId` cleared; `resolveForWorkspace` returns the global default).
- **Audit pass — Cancel buttons, status text, p-surface scale, edit-update path (Prompt 4 Phase E follow-up #3).** Stage-2 review of every dialog + form field surfaced more theme-propagation gaps and one UX mismatch. Closed:
  - Five labeled Cancel buttons (`ApplyPresetDialog`, `EditPresetDialog`, `ManageLayoutsDialog` editor row, `ManageWorkspacesDialog` editor row, `SaveLayoutAsDialog`) switched from `variant="ghost"` (transparent, no border by design) to `variant="secondary"` (bordered + filled-surface). Icon-only ghost buttons in row contexts kept as-is — a border would compete with the icon.
  - `TelemetryPanel.vue` status text switched from hardcoded `text-emerald-400` / `text-yellow-400` to semantic `text-success` / `text-warning` — OPEN / CONNECTING now follow the theme's status hues.
  - `ChromeSlot.vue` remove-badge hover switched from `hover:bg-red-500` (raw primitive) to `hover:opacity-90` — matches Button's `danger` opacity-driven hover pattern, theme-agnostic.
  - **PrimeVue surface scale generation.** Volt components (Dialog, Menu, Checkbox, InputText, Slider, DataView, Fieldset, SecondaryButton) consume `bg-surface-N` / `text-surface-N` / `border-surface-N` Tailwind utilities that resolve to `--color-p-surface-*`. `tokens.css` aliased those to `--color-slate-*`, so generated themes left every Volt-rendered surface (dialog backgrounds, the workspace-switcher menu, the customizer's own chrome, every form field) stuck on slate. Fixed by adding `--color-p-surface-0` through `-950` to `THEMEABLE_PRIMITIVE_TOKEN_NAMES` and emitting the 12-step scale derived from the user's base hue at very low chroma. Step 0 stays pure white literally (matches the `tokens.css` baseline); steps 50–950 are OKLCH with a whisper of base-hue tint.
  - **`updateExisting()` + "Update {name}" button in customizer edit mode.** Stage-2 review pointed out the UX mismatch: the original spec was "Edit current theme → save creates a new theme" (intentional "duplicate to edit"), but users naturally expect Edit to update in place. The dialog now shows TWO action buttons in edit mode — "Save as new theme" (the original behaviour, creates a copy) and "Update {name}" (calls `themeRepo.update` to overwrite the existing record in place, preserving id and workspace bindings). Create mode still shows just "Save as new theme". Edit-mode name pre-fill changed from `"{name} (Copy)"` to plain `{name}` so the primary Update action overwrites cleanly.
  - 1 new test in `generate.spec.ts` asserts the `--color-p-surface-0..950` scale is present, hue-preserving, and starts at literal `#ffffff` for step 0. Full suite **315/315**.
- **Accent-scale generation closes the wrapper-propagation gap (Prompt 4 Phase E follow-up).** `tokens.css` defines `--color-accent-50` through `-900` as aliases of `--color-blue-*`; 9 UI primitives (Button, IconButton, Input, Select, Tabs, Menubar, DataTable, ColorPicker, ColorSwatchPicker) plus `dockview.css` consume those aliases. The six built-in themes happen to use a blue interactive accent, so the default aliases match by coincidence — but a generated theme with any non-blue accent left every wrapper visually stuck on blue (Manage Layouts "Done" button, customizer "Save as new theme", entity list selection, panel borders, …). Fixed by:
  - Adding `--color-accent-50` through `-900` to `THEMEABLE_PRIMITIVE_TOKEN_NAMES` in `knownTokens.ts` — these aliases were always intended to be overridable (that's why they were aliases in the first place); the "primitives are off-limits" rule was for raw scales like `--color-blue-500`, not for the brand aliases.
  - `generateTheme()` now derives the full 10-step accent scale from the user's accent hue + chroma, varying lightness across a perceptually-balanced curve (L 0.97 → 0.28) with chroma capped at the extremes to stay in sRGB. Every wrapper that reads `bg-accent-500` / `var(--color-accent-*)` follows the chosen accent.
  - Customizer's Cancel button switched from `variant="ghost"` (transparent, no border by design) to `variant="secondary"` (bordered) — matches user expectation of a tappable secondary affordance.
  - Test bounds in `generate.spec.ts` widened from `[50, 60]` to `[60, 75]` to accommodate the 10 new tokens; new test asserts the scale's hue preservation + monotonic-darker lightness ordering.
- **Theme customizer dialog (Prompt 4 Phase E).**
  - `src/components/dialogs/ThemeCustomizerDialog.vue` — Linear-style authoring surface with 3–4 high-level inputs (base + accent colors via swatch grid, contrast slider 30–100, mode / density / optional font), a live preview that writes the generated tokens to a scoped `<div>` so the rest of the app isn't perturbed, and a contrast report surfacing the headline WCAG ratios + any failures. Opened from `View → Create new theme…` or `View → Edit current theme…`; the latter is only enabled when the active theme has `source === "generated"`.
  - Save always creates a new theme via `themeRepo.create` (auto-syncs `themeRegistry` via Phase C). "Edit current theme…" pre-fills from the active theme's `generation` block but still creates a new record — the original is untouched, so the pattern is "duplicate to edit". The "Generate paired variant" checkbox additionally creates the opposite-mode counterpart with `generation.paired` cross-linked in both directions, so the Light/Dark/Auto toggle bridges seamlessly. "Apply after saving" routes through `themeStore.setTheme()` so the workspace-binding contract still applies.
  - `src/components/ui/ColorSwatchPicker.vue` — OKLCH-typed swatch grid + optional native `<input type="color">` popover that converts hex → OKLCH on change. Sibling of the existing hex-typed `ColorPicker.vue` (which stays for operational use); the two are split by color space and intent per ADR 0002.
  - `src/modules/themes/curated-swatches.ts` — 10 base swatches (near-neutral with a whisper of tint, engine-sweet-spot lightness) + 10 accent swatches (mid-lightness, mid-chroma, evenly covering the hue wheel) + 6 curated font stacks (System UI, Inter, IBM Plex Sans, Geist, JetBrains Mono, IBM Plex Mono).
  - `MenuBar.vue` adds the two new View-menu items + the customizer dialog mount with edit-mode wiring.
  - 11 new tests across `tests/unit/themes/curated-swatches.spec.ts` (BLANK_DEFAULTS alignment, OKLCH validity, every accent round-trips through `generateTheme` cleanly) and `tests/unit/components/ui/ColorSwatchPicker.spec.ts` (render, aria-checked, click emits, whitespace-tolerant match, `allowCustom` toggle, disabled state).
- **Theme import / export core (Prompt 4 Phase D).**
  ...
  - Conflict policies trigger on **either** an id match OR a name match within `source: "imported"`. `abort` (default — returns `conflictWithExistingId` pointing at whichever record collided; id-match wins precedence), `rename` (mint fresh ULID + iteratively suffix the name " (Imported)" / " (Imported 2)" / … until unique), `replace` (delete whichever record(s) collide — by id and/or name — and create the imported one). The name-match path means re-importing the same JSON from a different session — different ULIDs but the same name — "just works" the way users expect, instead of failing on `themeRepo`'s invariant 2 at create time.
  - **Non-ULID ids are auto-minted with a warning.** Storage requires ULIDs (`themeRepo` invariant 1); imported files — especially LLM-authored or hand-written — often use friendly ids like `"my-cool-theme"`. The importer now silently reassigns to a fresh ULID and surfaces the change via `warnings`, so import "just works" for any JSON the schema accepts without compromising the storage layer's ULID guarantee. Auto-mint precedes conflict resolution (non-ULID id can't collide with anything stored).
...
- **Theme generation engine (Prompt 4 Phase B).**
  ...
  - [`docs/theme-generation-algorithm.md`](docs/theme-generation-algorithm.md) documents the per-scale math, contrast verification, gamut handling, limitations, where the engine refines the prompt's surface-elevation text to match the shipped built-ins, an **Extending the vocabulary** decision tree for adding new component / semantic tokens when the app grows new controls, and a **Beyond colors** section mapping the full taxonomy of themeable design dimensions (typography roles, motion, shape, elevation, effects, iconography, data-viz palettes, accessibility variants) with each pillar's current state in CommandVue and three operational-dashboard priorities (motion tokens, data-viz unification, high-contrast / CVD-safe variants). The `commandvue-theming-system` agent skill mirrors both recipes under Common operations so future agents adding controls or extending pillars hit the right path without reading the full doc.
...
- **Drag-and-drop reorder inside chrome edit mode.** Each chrome item becomes a draggable handle when edit mode is on; drop anywhere left or right of a sibling item to move it. Wired via `@atlaskit/pragmatic-drag-and-drop` and `@atlaskit/pragmatic-drag-and-drop-hitbox/closest-edge` (both already in the locked stack). An accent-colored vertical bar marks the insertion point on the side of the hovered target. Source item dims to 40% opacity while dragging. Cross-slot drag intentionally out of scope for this PR — the `canDrop` filter restricts to same-slot moves. Non-removable items (the always-on app icon) are anchored at their canonical position and don't drag.
