...
## Fonts (Google + system) — Theme Studio

The Theme Studio offers two font controls in one area:

- **Quick stack** (`<Select>`) — writes the legacy `fontFamily` string (System
  UI, common stacks). No network.
- **Google font picker** (`FontPicker`) — writes a structured **`fontSpec`** and
  loads the family at runtime.

A `fontSpec` is the lossless, portable shape persisted on a generated theme's
`base.input` and carried in the export JSON, so a re-import re-loads the same
font:

```ts
interface FontSpec {
  family: string; // catalog name, e.g. "IBM Plex Sans"
  source: "google" | "system" | "stack";
  weights?: number[]; // upright weights to request (default 400)
  fallback?: string; // appended after `family`
  // `heading` is reserved for C2 (Typography); C3 is body/sans only.
}
```

`fontSpec` is **google-only** today and, when present, is the source of truth —
the engine derives `--font-family-sans` / `--font-family-body` from it; the
quick-stack `fontFamily` is the fallback when no `fontSpec` is set. Picking one
control clears the other.

**Offline + catalog.** The picker lists ~1000 families from a bundled,
code-split snapshot (`src/assets/fonts/google-catalog.json`, regenerated by
`scripts/build-google-catalog.mjs`). Eight curated families are **self-hosted**
via `@fontsource/*` (`src/assets/fonts/local-fonts.css`; Inter reuses
`@fontsource-variable/inter`) so they render with **zero network**; non-curated
families load from Google when online and fall back to the system stack when
offline — never broken/tofu text.

**Security.** Family names are charset-allowlisted at the Zod edge _and_ in the
loader; the css2 URL is built in one place with the weight clamped to the
catalog; the only injected origins are `fonts.googleapis.com` (stylesheet) and
`fonts.gstatic.com` (woff2). Loading the full catalog at runtime requires those
two CSP origins — see **[Deployment → Google Fonts](./deployment.md#google-fonts-theme-studio)**.
