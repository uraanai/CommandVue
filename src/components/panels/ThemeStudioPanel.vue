 ...
 *  - An in-panel **preview pane** (right column on wide panels, stacked when
 *    narrow) renders a scoped sample of the theme being built — always on, so
 *    there is immediate visual feedback even before the app recolors.
 *  - **Live across app** (toggle, default on) additionally writes the generated
 *    tokens to the top-window root via `themeStore.previewThemeTokens`, so the
 *    main window + every float + every pop-out recolor at once (the A2a headline;
 *    targets the captured `APP_ROOT`, so it works even from a popped-out Studio).
 *    Turn it off to keep the rest of the app stable while you experiment.
 *
 * Idle by default: a blank panel does not push a preview to the app on mount (so
 * a restored layout doesn't recolor just because the Studio was left open). Save
 * persists + applies; Discard / unmount cancels the app preview.
 */
...
/** Scoped style for the in-panel preview pane — the freshly-generated tokens as
 *  inline custom properties, cascading to the sample's descendants. */
const previewStyle = computed<Record<string, string>>(() => ({
  ...(a.generationResult.value?.tokens ?? {}),
}));

/** When on, edits also recolor the whole app (every window). */
const liveAcrossApp = ref(true);
/** Whether the user has interacted yet (gates the app-wide recolor on mount). */
...
onMounted(() => {
  ...
  if (theme) {
    interacted = true;
    applyToApp(); // editing an existing theme previews it across the app immediately
  }
});

// Any regeneration after mount is a user action → push to the app if enabled.
watch(a.generationResult, () => {
  interacted = true;
  applyToApp();
});

// Toggling the app-wide preview pushes or withdraws it without losing the draft.
watch(liveAcrossApp, (on) => {
  if (on) {
    if (interacted) applyToApp();
  } else {
    themeStore.cancelPreview(); // restore the committed theme across the app
  }
});
...
async function onSave(): Promise<void> {
  ...
  if (saved) {
    themeStore.endPreview(); // the committed theme is applied; drop the overlay
    interacted = false;
  }
}
...
<template>
  <div class="bg-surface text-foreground flex h-full w-full flex-col overflow-hidden">
    ...
    <!-- @container so the columns respond to the PANEL width, not the window. -->
    <div class="@container min-h-0 flex-1 overflow-hidden">
      <div class="grid h-full grid-cols-1 @2xl:grid-cols-[20rem_1fr]">
        <!-- Controls column -->
        <div
          class="@2xl:border-border-subtle flex min-h-0 flex-col gap-4 overflow-y-auto p-3 text-sm @2xl:border-r"
        >
        ...
        <!-- Preview column -->
        <div class="bg-surface-sunken flex min-h-0 flex-col gap-3 overflow-y-auto p-3">
          <span class="text-faint text-[10px] tracking-wider uppercase">Live preview</span>
          <!-- Scoped sample: `previewStyle` sets the generated tokens as inline
               custom properties; `data-density` re-binds the density cascade;
               `font-family-body` makes the font control visible here. -->
          <div
              ...
              <div class="flex flex-wrap items-center gap-2">
                <span
                  v-for="s in [
                    ...
                    { label: 'INFO', fg: '--color-status-info', bg: '--color-status-info-subtle' },
                  ]"
                  :key="s.label"
