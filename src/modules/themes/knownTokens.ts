...
export const THEMEABLE_PRIMITIVE_TOKEN_NAMES = [
  ...
  "--font-family-sans",
  "--font-family-mono",
  // Heading role (Track A C2). C2 owns this token — C3 must NOT re-declare it
  // (the no-duplicate guard in knownTokens.spec.ts enforces single ownership).
  "--font-family-heading",
...
/** Type-scale size primitives (Track A C2). Length-valued (rem). Generated by
 *  the modular type-scale deriver; default fixed values live in `tokens.css`. */
...
/** Type-scale line-height companions (Track A C2). Unitless number values; paired
 *  one-per-size with {@link TYPE_SCALE_TOKEN_NAMES} (Tailwind v4 `--text-*--line-height`). */
