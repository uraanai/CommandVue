...
**Comprehensive theming overhaul (Track A) — SPEC COMPLETE, awaiting maintainer review**
Full phased design + execution spec authored at [`specs/track-a-theming.md`](./specs/track-a-theming.md): sub-phases A1a (surface-seam fix) · A1a.5 (v2 data model + schema/DB bump) · A1b (status/toast unlock) · A1c (richer palette) · A2a (Theme Studio panel) · A2b (per-token editor) · A3 (duotone icons), each independently shippable, plus a migration strategy and 17 open maintainer decisions (with recommended defaults). Key code-verified finding: the surface seam is compounded by a dead-variable bug — the generator + `knownTokens` target `--color-p-surface-*`, which nothing reads; Volt actually resolves single-`p` `--p-surface-*`. Execution is gated on maintainer sign-off of the spec's open decisions.
