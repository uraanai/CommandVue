...
Buttons render via dockview's per-group **header-actions** component (verify the dockview-vue right-actions slot during design). **Close All** is near-native (iterate `group.panels` → `api.removePanel`). **Minimize-to-tray is custom**: a `minimized` flag + restore target persisted on `PanelState.state` (mirroring `headerless` / `floatPrevHeaderless`), a new bottom-left tray component above the status bar, and collapse/restore session actions — and it must interoperate with clean panes, floats, and maximize. Needs a research + design pass (tray placement/overflow, restore semantics, persistence, interactions) before implementation, like Phase 3. Sequenced AFTER Phase 3b/3c. Status: requested by maintainer; registered, design pending.
