...
**Comprehensive theming overhaul (Track A) — SPEC COMPLETE + EXTERNALLY VALIDATED, awaiting maintainer go-ahead to execute A1a**
Two planning docs, both kept in the repo:

- **Execution plan (authoritative):** [`specs/track-a-theming.md`](./specs/track-a-theming.md) — sub-phases A1a (surface-seam fix) · A1a.5 (v2 data model + schema/DB bump) · A1b (status/toast unlock) · A1c (richer palette) · A2a (Theme Studio panel) · A2b (per-token editor) · A3 (duotone icons), each independently shippable, plus a migration strategy and 17 open maintainer decisions (with recommended defaults). Code-verified finding: the surface seam is compounded by a dead-variable bug — the generator + `knownTokens` target `--color-p-surface-*`, which nothing reads; Volt resolves single-`p` `--p-surface-*`.
- **Research archive (reference only, settled — do NOT re-run):** [`specs/track-a-theming-single-source.md`](./specs/track-a-theming-single-source.md) — an external "another eye" pass that pressure-tested the architecture against PrimeVue-native theming + Material 3 / MUI / Ant / Chakra / Mantine.

**Architecture decision — SETTLED, do not re-evaluate:** **Option C** (CommandVue's custom `--color-*` is the single source of truth; PrimeVue/Volt is a derived consumer). Adopting PrimeVue 4's native styled preset (Option B) is **rejected** on verified grounds (styled preset ⊥ unstyled mode per PrimeVue docs; `@primeuix/themes` not installed; would abandon Volt/`:pt` + ADR-0002; would still not own app/dockview surfaces; discards the OKLCH AA generator). Four net-new additions from the research edition are **adopted into execution**: a CI single-source guard, ADR-0003, the `--cv-float-tint`/`--dv-*` re-chain, and on-color status pairing via a central variant resolver. **Future sessions: the research is done and the architecture is final — do not re-ask or re-run it.** Execution of A1a is gated only on the maintainer's go-ahead + sign-off of the open decisions.
...
**Dockview windowing — clean panes, tab controls, float/pop-out, minimize/maximize — ✅ COMPLETE**
All phases merged: clean panes (#102) · tabbed context menu + maximize/restore (#103) · floating windows + glass opacity (#105) · header actions + float-maximize + minimize-to-tray (#109/#110/#124) · 3D camera persistence (#123) · pop-out + theme-mirroring (#125) · per-tab vs per-group pop-out (#126) · native pop-out-local context menu (#127) · cross-window "Send to window" relocation (#128). Runtime-verified that Cesium/MapLibre WebGL contexts survive both the pop-out and cross-window moves in Chromium, so the planned map-reinit phase (6b) was unnecessary. Track A's Theme Studio now has its windowing prerequisite.

Today a dock tab has only a close button, and every group always shows a tab strip. Track B adds VS Code-style windowing: **clean (header-less) content panes** — any number, freely arranged (Cesium + MapLibre side by side, 2-up, 3+1), with no tab strip eating space; a per-group **Hide/Show header** toggle (splits inherit clean mode); per-tab close + minimize and per-header maximize/float/pop-out on normal panels; a right-click tab context menu; **maximize**, **minimize** (to **floating bars** stacked bottom-left above the status bar), in-window **float**, and separate-browser-window **pop-out** (with the theme mirrored into the child window); and **cross-window relocation** ("Send this view to that window") — a content move, since a true window-role transfer is infeasible (pop-outs are render targets of the one dock engine), gated on map/3D surviving the cross-window move. Verified against `dockview-core@6.6.1`. **Sequenced first** — it is a hard prerequisite for Track A's pop-outable Theme Studio. Full design + 6-phase plan: [`specs/track-b-dockview-windowing.md`](./specs/track-b-dockview-windowing.md). **Progress:** Phase 1 (clean panes) merged (#102); Phase 2 (tabbed-pane context menu + Maximize/Restore + tab-overflow-dropdown fix + Split removed in favor of Add-Component + drag-to-split) in review (#103). Large, phased.

**Floating overlay windows with map-see-through opacity (Track B Phase 3) — NEW**
Pop a docked panel out of the grid into an in-window floating window that sits on top of the app; drag/resize it anywhere; re-dock by drag or a **Dock back** menu item; and control its **background opacity** so the Cesium/MapLibre map underneath shows through (content stays full-color — a translucent _background_, never element opacity). Triggered from the right-click dock context menu (**Float window** / **Dock back**); opacity is controlled by a **continuous slider** in the floating window's header (dark-glass tint in both light + dark themes). The windowing is ~100% native dockview (`addFloatingGroup` / `moveTo` / `location.type==='floating'` / `toJSON` persists geometry — all verified against `dockview-core@6.6.1`); the see-through opacity is the one custom piece (a `--cv-float-alpha` CSS var + a `color-mix` background scoped to `.dv-groupview-floating`, persisted like `headerless` in `PanelState.state`). Inspired by the Osiris app's "tactical glass" panels over a map. Separate-browser-window pop-out and frosted blur are deferred; minimize-to-tray is registered as its own Phase 4 task (below). Full design + 3-sub-phase plan (3a float/dock-back + WebGL-survival gate · 3b opacity control · 3c persistence hardening): [`specs/track-b-phase-3-floating-windows.md`](./specs/track-b-phase-3-floating-windows.md). Status: **Phase 3a (float / dock-back, WebGL-survival runtime-verified) shipped — #105.** Maintainer decisions settled (dark glass in both themes; continuous opacity slider). **Phase 3b (opacity slider) is next**, then 3c (persistence hardening).

**Tab-group header actions — Close All + Minimize-to-tray (Track B Phase 4) — NEW**
Two action buttons on the FAR RIGHT of a tabbed group's header (dockview's per-group right-actions area), beside the existing per-tab close button:

- **Close All** — closes every panel in the group in one click (empty-workspace-guarded, like `removePanelGuarded`); a group-level complement to per-tab close.
- **Minimize** — collapses the WHOLE group into a single labelled bar in a **minimized-groups tray docked bottom-left, just above the status bar**; clicking the bar restores the group to its prior dock position. Multiple minimized groups stack left-to-right. This realizes the "minimize to floating bars" item from the original Track B spec.

Buttons render via dockview's per-group **header-actions** component (one unified `CommandVueHeaderActions.vue` branched on `location.type`). The phase also gained the **floating-window header row** the maintainer asked for: a custom **Maximize** (fill-screen ⇄ restore, NOT dockview's grid-only maximize) + Close beside the Phase 3b opacity eye. **Close All** is near-native (iterate `group.panels` → `api.removePanel`, empty-workspace-guarded). **Minimize-to-tray is custom**: an **ephemeral** in-memory `minimized` store (cleared on `loadLayout` — D6's v1 decision; NOT persisted, which also dodges unstable group ids), a bottom-left `MinimizedDock` overlay mounted in `AppShell` above the status bar, and capture/restore session actions interoperating with clean panes, floats, and maximize. Design pass complete — see [`specs/track-b-phase-4-header-actions.md`](./specs/track-b-phase-4-header-actions.md). Sub-phases: **4a Close All** (component consolidation + `closeAllInGroup`), **4b Float Maximize**, **4c Minimize-to-tray**. Status: **4a shipped (#109)** (group Close All + group-scoped confirm + tab-matched icon). **4b shipped (#110)** (float-header Maximize fill ⇄ restore + Close, via dockview's internal `floatingGroups…position()`, persisted; WebGL-survival + persistence verified). **4c in progress** (this PR, completes Phase 4): **Minimize-to-tray** — a Minimize button on grid (beside Close All) + float headers collapses the whole group into a bottom-left tray bar (`MinimizedDock`/`MinimizedBar` + an in-memory `minimized` store); clicking restores the group by re-adding its panels by id (the PanelState records survive removal, so per-panel state round-trips). Ephemeral (cleared on `loadLayout`); dirty-neutral (the deferred `onDidLayoutChange` microtask is re-cleared). Maintainer decisions settled (ephemeral minimize; sub-phase order 4a→4b→4c).
