...
> ⛔ **STATUS — RESEARCH ARCHIVE / REFERENCE ONLY · SETTLED 2026-06-02 · DO NOT RE-RUN OR RE-OPEN.**
>
> This document is the output of a one-time external + internal research pass (an "another eye" review) that pressure-tested Track A's architecture against how mature systems achieve single-source theming — PrimeVue 4's native design-token preset, Material Design 3 color roles, MUI / Ant Design / Chakra / Mantine theme objects, real admin-panel theme customizers, and the Tailwind/CSS-variable single-namespace pattern. **The research is complete; the decision is final.** It is NOT a separate execution track.
>
> **Decision (SETTLED — do not reopen):** the architecture in the authoritative execution spec [`track-a-theming.md`](./track-a-theming.md) is correct — **Option C**: CommandVue's custom `--color-*` namespace is the single authored source of truth, and PrimeVue/Volt is a mechanically-derived consumer (matching what MUI/Ant/Chakra/Mantine all do — none subordinates its source of truth to a UI dependency). **Option B (adopt PrimeVue 4's native styled preset as the source) is REJECTED on verified grounds:** PrimeVue's styled preset and unstyled mode are mutually exclusive (in unstyled mode the design-token CSS variables are not emitted — verified via PrimeVue docs); `@primeuix/themes` is not installed and the app runs `unstyled: true` with no preset; adopting it would abandon Volt/`:pt` and violate the locked stack + ADR-0002; it would still not own the app body / dockview / float surfaces (trading one seam for another); and it discards the OKLCH AA-solving generator that lets a non-technical user pick any color and stay legible.
>
> **➡️ For future AI sessions:** do NOT propose re-running this research or its workflows, do NOT re-ask whether to adopt PrimeVue-native theming, and do NOT treat this edition's contents as un-done or "not yet included." This pass HAS been run; its conclusions are decided history. The edition's FOUR net-new additions over the execution spec are **ADOPTED** and applied during execution alongside `track-a-theming.md`: (1) a CI single-source guard (no raw color literals / unknown `--*` tokens); (2) ADR-0003 recording Option C vs B with evidence; (3) re-chaining the theme-blind `--cv-float-tint` / `--dv-*` refs into A1a; (4) on-color status pairing (`--color-status-*-fg`/`-border`) via a central severity variant resolver. This file is retained for provenance + the comparison table only.
>
> _Historical header:_ Owner: Lead Architect (single-source lens) · Relationship: a governance-and-completeness overlay on `track-a-theming.md` (same Option C, same engine, same migration, same critical path) · Depends on: Track B (pop-out theme-mirroring, shipped) · Schema target: `THEME_SCHEMA_VERSION` 1 → 2 · DB target: `DB_VERSION` 2 → 3 · New ADR: `docs/decisions/0003-theme-single-source-of-truth.md`
