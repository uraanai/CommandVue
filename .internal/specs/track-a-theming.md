...
- **A1a.5 (the v2 model) is the keystone** and a **standalone PR** (Open Decision 8) so the CSS-only seam fix and the higher-risk schema/DB migration are reviewed/reverted independently. It must land **before A1b** (so `statusHues`/`statusOverrides` have a home in `base.input`, avoiding a second bump) and **before A1c** (so diff-into-overrides + §3i neutralize the engine change).
  > **⚠ Reconciliation note (2026-06-03):** this ordering was **inverted in practice** — A1b shipped (PR #133) ahead of A1a.5, so `statusOverrides` is bolted onto the v1 `ThemeGenerationMeta` (`src/types/theme.ts:88`, `schemaVersion: 1`) and the schema/DB bumps never landed. A1a.5 is therefore no longer greenfield: its migration must **retroactively upcast already-persisted v1 themes carrying `generation.statusOverrides`** into the v2 `base`+`overrides` model. See [`track-a-post-a1b-ux-systems.md`](./track-a-post-a1b-ux-systems.md) §Keystone-ordering reconciliation for the full analysis and revised A1a.5 scope.
